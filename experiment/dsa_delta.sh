...
case "$(uname -s)" in
  Darwin)
    echo "computing DSA delta on macOS"
    for item in "${list[@]}" ; do
      set -- $item
      echo $1
      echo "Computing DSA delta - text: $1"
      /usr/bin/time -l ../build/delta -i ${output_data_path}/$1.dsa -c int64_t >> ${output_log_path}/dsa_delta.log 2>&1
    done
    ;;

  Linux)
    echo "computing DSA delta on Linux"
    # Linux固有のコマンド
    for item in "${list[@]}" ; do
      set -- $item
      echo $1
      echo "Computing DSA delta - text: $1"
      nohup /usr/bin/time -f "#build dsa_delta, $1, %e sec, %M KB" ../build/delta -i ${output_data_path}/$1.dsa -c int64_t >> ${output_log_path}/dsa_delta.log 2>&1
    done
    ;;
