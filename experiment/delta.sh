...
case "$(uname -s)" in
  Darwin)
    echo "computing delta on macOS"
    for item in "${list[@]}" ; do
      set -- $item
      echo $1
      echo "Computing delta - text: $1"
      /usr/bin/time -l ../build/delta -i ${dataset_folder_path}/$1 -c uint8_t >> ${output_log_path}/delta.log 2>&1
    done
    ;;

  Linux)
    echo "computing delta on Linux"
    # Linux固有のコマンド
    for item in "${list[@]}" ; do
      set -- $item
      echo $1
      echo "Computing delta - text: $1"
      nohup /usr/bin/time -f "#build delta, $1, %e sec, %M KB" ../build/delta -i ${dataset_folder_path}/$1 -c uint8_t >> ${output_log_path}/delta.log 2>&1
    done
    ;;
