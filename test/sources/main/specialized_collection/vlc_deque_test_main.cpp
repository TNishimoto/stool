...
void test_randomized(uint64_t N, uint64_t max_val, uint64_t num_ops, uint64_t seed, bool verbose = false)
{
    ...
    for (uint64_t op = 0; op < num_ops; ++op)
    {
        ...
        else if (act == 5 && !stddeq.empty())
        { // remove
            size_t pos = safe_rand_index(stddeq.size());
            if (verbose)
                std::cout << "remove, pos: " << pos << std::endl;
            stddeq.erase(stddeq.begin() + pos);
            vdeq.remove(pos);
        }
        else if (act == 6 && !stddeq.empty())
        { // set_value
            size_t pos = safe_rand_index(stddeq.size());
            uint64_t w = stddeq[pos];
            uint64_t v = mt() % w;
            if (verbose)
                std::cout << "set_value," << pos << ", " << v << ", " << w << ", " << stddeq[pos] << ", " << vdeq.at(pos) << std::endl;
            assert(stddeq[pos] == vdeq.at(pos));
            stddeq[pos] = v;
            vdeq.set_value(pos, v);
        }
        else if (act == 7 && stddeq.size() > 1)
        { // shift_left
            size_t k = 1 + (mt() % (stddeq.size() - 1));
            if (verbose)
                std::cout << "shift_left, pos: " << k << std::endl;
            stddeq.erase(stddeq.begin() + k);
            vdeq.shift_left(k);
        }
        else if (act == 8 && stddeq.size() > 1)
        { // shift_right
            size_t k = 1 + (mt() % (stddeq.size() - 1));
            if (verbose)
        ...
        // Test psum, reverse_psum, search, and at methods with random arguments

        if (!stddeq.empty())
        {
            // Choose a random position for at, psum, reverse_psum, and search tests
            size_t size = stddeq.size();
            size_t pos = safe_rand_index(size);

            // Test at
            uint64_t std_val = stddeq[pos];
            uint64_t vlc_val = vdeq.at(pos);
            if (verbose)
                std::cout << "at(" << pos << ") = " << std_val << ", " << vlc_val << std::endl;
            assert(std_val == vlc_val);

            // Test psum (prefix sum: sum of values from 0 to pos-1, inclusive)
            uint64_t std_psum = 0;
            for (size_t i = 0; i <= pos; ++i)
            {
                std_psum += stddeq[i];
            }
            uint64_t vlc_psum = vdeq.psum(pos);
            if (verbose)
                std::cout << "psum(" << pos << ") = " << std_psum << ", " << vlc_psum << std::endl;
            
            assert(std_psum == vlc_psum);

            // Test reverse_psum (sum from pos to end)
            uint64_t std_rpsum = 0;
            for (size_t i = 0; i <= pos; i++)
            {
                std_rpsum += stddeq[size - i-1];
            }
            uint64_t vlc_rpsum = vdeq.reverse_psum(pos);
            if (verbose)
                std::cout << "reverse_psum(" << pos << ") = " << std_rpsum << ", " << vlc_rpsum << std::endl;
            assert(std_rpsum == vlc_rpsum);

            // Test search (find the smallest index s.t. psum(i) > target)
            // For testing, pick target between 0 and sum of all elements + 1
            uint64_t total = 0;
            for (size_t i = 0; i < size; ++i)
                total += stddeq[i];
            if (total > 0)
            {
                uint64_t target = mt() % (total + 1);

                // Linear scan for stddeq
                uint64_t acc = 0;
                size_t found = size;
                for (size_t i = 0; i < size; ++i)
                {
                ...
                size_t vlc_found = vdeq.search(target);
                if (verbose)
                    std::cout << "search(" << target << ") = " << found << ", " << vlc_found << std::endl;
                assert(found == vlc_found);
            }
        }
    }
    for (size_t i = 0; i < stddeq.size(); ++i)
...
int main()
{
    ...
    test_randomized(5, 65536, 10000, 12345);
