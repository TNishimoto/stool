...
int main(int argc, char *argv[])
{
    ...
    for (uint64_t alphabet_type = 0; alphabet_type < max_alphabet_type; alphabet_type++)
    {
        ...
        for (uint64_t x = 0; x < 100; x++)
        {
            ...
            std::vector<uint64_t> sa1 = stool::sais_suffix_array(text);
            std::vector<uint64_t> sa2 = stool::ArrayConstructor::construct_naive_suffix_array(text);
            stool::EqualChecker::equal_check(sa1, sa2, "test SA");
