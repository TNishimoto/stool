...
void equal_test(const stool::NaiveArrayForFasterPsum<SIZE> &dequeA, const std::vector<uint64_t> &dequeB)
{
...
void access_test(uint64_t max_len, uint64_t alphabet_size, uint64_t number_of_trials, uint64_t seed, int message_paragraph = stool::Message::SHOW_MESSAGE)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        while (len < max_len)
        {
            ...
            stool::NaiveArrayForFasterPsum<SIZE> deque(seq);
...
{
    stool::NaiveArrayForFasterPsum<SIZE> deque;
