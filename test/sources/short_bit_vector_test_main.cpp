...
std::vector<bool> create_random_bit_vector(uint64_t len, std::mt19937_64 &mt64)
{
...
void access_test(uint64_t max_len, uint64_t number_of_trials, std::mt19937_64 &mt64)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        while (len < max_len)
        {
            std::vector<bool> bv = create_random_bit_vector(len, mt64);
...
{    
    ...
    for(uint64_t i = 0; i < number_of_trials; i++){
        ...
        for(uint64_t j = 0; j < max_len; j++){
            std::vector<bool> bv = create_random_bit_vector(max_len, mt64);
            stool::ShortBitVector short_bv(bv);
...
{    
    ...
    for(uint64_t i = 0; i < number_of_trials; i++){
        ...
        for(uint64_t j = 0; j < max_len; j++){
            ...
            std::vector<bool> bv = create_random_bit_vector(max_len, mt64);
            stool::ShortBitVector short_bv(bv);
            std::vector<bool> replace_bv = create_random_bit_vector(insert_length, mt64);
...
{ 
    ...
    for(uint64_t i = 0; i < number_of_trials; i++){
        ...
        for(int64_t j = 0; j < (int64_t)max_len - (int64_t)erase_length; j++){
            std::vector<bool> bv = create_random_bit_vector(max_len, mt64);
            stool::ShortBitVector short_bv(bv);
            std::vector<bool> replace_bv = create_random_bit_vector(erase_length, mt64);
            uint64_t insert_value = 0;
...
void replace_test(uint64_t max_len, uint64_t number_of_trials, uint64_t replace_length, std::mt19937_64 &mt64)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        for (uint64_t j = 0; j < short_bv.size() - replace_length; j++)
        {
            ...
            std::vector<bool> replace_bv = create_random_bit_vector(replace_length, mt64);
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char *argv[])
{
    ...
    access_test(seq_len, 100, mt64);
    rank_test(seq_len, 100, mt64);
    select_test(seq_len, 100, mt64);
    push_back_and_pop_back_test(seq_len, 100, mt64);
    replace_test(seq_len, 100, mt64);
    extend_right_test(seq_len, 10, mt64);
    
    insert_test(seq_len, 10, mt64);    
    erase_test(seq_len, 10, mt64);
}
