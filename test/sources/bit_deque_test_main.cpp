...
void equal_test(const stool::BitDeque &bv, const std::vector<bool> &naive_bv)
{
    if(naive_bv.size() != bv.size()){
        ...
        std::cout << "naive_bv = " << to_string(naive_bv) << std::endl;
        std::cout << "      bv = " << bv.to_string() << std::endl;
...
void push64_and_pop64_test(uint64_t max_len, uint64_t number_of_trials, uint64_t seed)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        while (naive_bv.size() < max_len)
        {
            ...
            }else if(type == 4 && naive_bv.size() >= len){

                bit_deque.pop_back(len);
                for(int64_t j = 0; j < len; j++){
                    naive_bv.pop_back();
                }
            }else if(type == 5 && bit_deque.size() >= len){
...
void insert_test(uint64_t max_len, uint64_t number_of_trials, uint64_t seed)
{
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char *argv[])
{
    ...
    insert_test(seq_len, number_of_trials, seed);
    /*
